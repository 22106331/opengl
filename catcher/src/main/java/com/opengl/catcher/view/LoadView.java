...
import com.opengl.catcher.thread.SwitchThread;
import com.opengl.catcher.util.CLpng;
import com.opengl.catcher.util.LoadUtil;
...
public class LoadView extends BNAbstractView{
	...
	List<BN2DObject> load=new ArrayList<BN2DObject>();//���BNObject����
	List<BN2DObject> loadjm=new ArrayList<BN2DObject>();//���BNObject����
	MySurfaceView mv;
	int initIndex=0;
	int index=0;
	...
	{
		TextureManager.loadingTexture(this.mv, 0, 92);//����ͼƬ��Դ
		for(int i=1;i<=5;i++)
		{
			for(int j=1;j<=5;j++)
			{
